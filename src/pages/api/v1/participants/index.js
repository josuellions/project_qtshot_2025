...
export default async function participants(req, res) {
  const query = `SELECT json_object_agg(date_event_str, event_date ORDER BY date_event_str DESC) AS events    FROM (
      SELECT
        TO_CHAR(dateAt::date, 'YYYY-MM-DD') AS date_event_str,
      ...
      FROM participants
      GROUP BY dateAt::date
      ORDER BY dateAt::date DESC
    ) AS daily_date
  `;

...
describe("GET to /api/v1/status", () => {
  describe("Anonymous user", () => {
    test("Retrieving current system status", async () => {
      ...
      expect(resposeBody.dependecies.database.version).toEqual(
        `${process.env.POSTGRES_VERSION}`,
      );

      expect(resposeBody.dependecies.database.max_connections).toEqual(
        parseInt(`${process.env.POSTGRES_MAX_CONNECTIONS}`),
      );

      expect(resposeBody.dependecies.database.opened_connections).toEqual(
